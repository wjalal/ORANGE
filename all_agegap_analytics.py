...
if len(sys.argv) >= 6:
    lpo_sp = sys.argv[5]
    split_id_r1 = sys.argv[6]
    split_id_r2 = sys.argv[7]
else:
    lpo_sp = ""
    split_id_r1 = split_id
    split_id_r2 = split_id

with open('gtex/organ_list.dat', 'r') as file:
    tissues = ["agegap_" + line.strip() for line in file]


# cols = ['max_agegap', 'min_agegap']
cols = tissues + ['max_agegap']
...
if lpo_sp == "":
    if regr == "lasso":
        all_tissue_res = pd.read_csv(filepath_or_buffer="gtex_outputs/lasso_PTyj_nma_tstScale_redc" + gene_sort_crit + "_train_bs" + n_bs + "_" + split_id + ".tsv", sep='\t').set_index("SUBJID")
    elif regr == "ridge":
        all_tissue_res = pd.read_csv(filepath_or_buffer="gtex_outputs/ridge_PTyj_nma_tstScale_redc" + gene_sort_crit + "_train_bs" + n_bs + "_" + split_id + ".tsv", sep='\t').set_index("SUBJID")
    elif regr == "elasticnet":
        all_tissue_res = pd.read_csv(filepath_or_buffer="gtex_outputs/elasticnet_PTyj_nma_tstScale_redc" + gene_sort_crit + "_train_bs" + n_bs + "_" + split_id + ".tsv", sep='\t').set_index("SUBJID")
    elif regr == "logistic":
        all_tissue_res = pd.read_csv(filepath_or_buffer="gtex_outputs/logistic_PTyj_f1ma_tstScale_redc" + gene_sort_crit + "_train_bs" + n_bs + "_" + split_id + ".tsv", sep='\t').set_index("SUBJID")
    elif regr == "randomforest":
        all_tissue_res = pd.read_csv(filepath_or_buffer="gtex_outputs/randomforest_PTyj_nma_tstScale_redc" + gene_sort_crit + "_train_bs" + n_bs + "_" + split_id + ".tsv", sep='\t').set_index("SUBJID")
    elif regr == "svr":
        all_tissue_res = pd.read_csv(filepath_or_buffer="gtex_outputs/svr_PTyj_nma_tstScale_redc" + gene_sort_crit + "_train_bs" + n_bs + "_" + split_id + ".tsv", sep='\t').set_index("SUBJID")
    elif regr == "pls":
        all_tissue_res = pd.read_csv(filepath_or_buffer="gtex_outputs/pls_PTyj_nma_tstScale_redc" + gene_sort_crit + "_train_bs" + n_bs + "_" + split_id + ".tsv", sep='\t').set_index("SUBJID")
else:
    all_tissue_res = pd.read_csv(filepath_or_buffer="gtex_outputs/bs_agegaps_zscpred_pls_redc" + gene_sort_crit + "_train_bs" + n_bs + "_" + split_id_r1 + "-" + split_id_r2 + lpo_sp + ".tsv", sep='\t').set_index("SUBJID")
...
all_tissue_res = all_tissue_res.dropna(thresh=min_non_null_columns)
...
for col in all_tissue_dth_agegap:
    ...
    for i in range(0,4):
        p_gt = all_tissue_dth_agegap[col][i]['p_gt'] / (int(split_id_r2[3:])-int(split_id_r1[3:])+1)
        p_lt = all_tissue_dth_agegap[col][i]['p_lt'] / (int(split_id_r2[3:])-int(split_id_r1[3:])+1)
        p_d = all_tissue_dth_agegap[col][i]['p_d'] / (int(split_id_r2[3:])-int(split_id_r1[3:])+1)
        p_r = all_tissue_dth_agegap[col][i]['p_r'] / (int(split_id_r2[3:])-int(split_id_r1[3:])+1)
