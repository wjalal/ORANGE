...
def Train_all_tissue_aging_model_randomforest(md_hot_train, df_prot_train,
    ...
    for tissue in tissues:
        ...
        # print (md_hot_train_tissue)
        smp_attr = get_samp_attr(tissue=tissue)
        smp_attr = smp_attr.merge(right = df_prot_train_tissue.index.to_series(), how='inner', left_index=True, right_index=True)
        print(smp_attr)

        df_X_train = pd.concat([md_hot_train_tissue[["SEX"]], smp_attr[["SMATSSCR", "SMTSISCH"]], df_prot_train_tissue], axis=1)
        df_X_train[['SMATSSCR', 'SMTSISCH']] = df_X_train[['SMATSSCR', 'SMTSISCH']].fillna(df_X_train[['SMATSSCR', 'SMTSISCH']].median())
        df_Y_train = md_hot_train_tissue[["AGE"]].copy()
        print(df_X_train)  # This will print the first 10 columns of the DataFrame
        ...
        tmp = scaler.transform(df_X_train)
        df_X_train = pd.DataFrame(tmp, index=df_X_train.index, columns=df_X_train.columns)
        print (df_X_train)
