...
def Bootstrap_train(df_X_train, df_Y_train, train_cohort,
    ...
    lasso = Ridge(random_state=0, tol=0.01, max_iter=50000)
    ...
    # Retrain 
    lasso = Ridge(alpha=best_alpha, random_state=0, tol=0.01, max_iter=50000)
    lasso.fit(X_train_sample, Y_train_sample)
    print ("lasso retrained.. (seed = ", seed, ")")
    # SAVE MODEL
    savefp="gtex/train_splits/train_bs" + n_bs + "_" + split_id + "/data/ml_models/"+train_cohort+"/"+agerange+"/"+norm+"/"+tissue+"/"+train_cohort+"_"+agerange+"_"+norm+"_ridge_"+tissue+"_seed"+str(seed)+"_aging_model.pkl"
